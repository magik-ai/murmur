...
win, because the second push is rejected as non-fast-forward.
"""
...
def default_branch(cfg):
    ...
    Asked, never assumed. `main` used to be hardcoded, so a head office on
    `master` - or on anything a team happens to use - was simply wrong.
    """
...
def ensure_cache(cfg=None):
    ...
    Lazy on purpose. `hq install` used to create it and then symlink
    `<cache>/bin/hq` into the bin dir, which assumed every head office repo was
    a copy of hq's own source - a head office that holds claims and mailboxes
    and nothing else got a traceback. Installing hq and caching the office are
    two different jobs, and only the commands that actually read the office pay
    for the clone.
    """
...
def commit_claims_change(writes, deletes, message):
    """Commit claim-file changes to the claims branch via index plumbing - the
    cache clone NEVER checks out another branch (it is also the CLI's home, so
    a checkout would delete the running tool). Push is the compare-and-swap;
    one retry on a lost race."""
    cfg = load_config()
    for attempt in (1, 2):
        ...
        try:
            ...
            # cache, a state dir hq may not write, a machine with no git. Every
            # one of them used to be a stack trace out of a plumbing command
            # nobody but hq has heard of, and none of them reached the office,
            # so the claim plainly did not happen.
        ...
        if push.returncode == 0:
            return
        if attempt == 2:
            sys.exit(f"hq: lost the claims race twice - inspect manually:\n{push.stderr}")
        # Lost the race: someone pushed first. Re-fetch and re-evaluate.
...
def cmd_claim(args):
    ...
    existing = claims_tree(cfg).get(path)
    if existing and active(existing) and existing.get("owner") not in (me,):
        sys.exit(
            f"hq: {repo}#{args.branch} is CLAIMED by {existing['owner']} "
            f"until {existing.get('expires', '?')} - talk first: "
            f"hq msg {existing['owner']} \"...\""
        )
    claim = {
...
def cmd_release(args):
    ...
    if not existing:
        ...
        return
    if existing.get("owner") != me and not args.force and not is_sovereign(me, cfg):
        sys.exit(f"hq: claim belongs to {existing['owner']} - use --force to override")
...
def cmd_check_push(args):
    ...
    here does not fail one push, it freezes every push on the machine until
    somebody debugs a traceback. Three rounds of review found three different
    exceptions escaping this command - a malformed config, an unreadable one, a
    config that is not UTF-8 - each fixed where it arose. This is the general
    case, because the next one has not been thought of yet: whatever hq did not
    foresee becomes one warning line and an open gate, since a crash is not
    evidence of a claim. A deliberate block is a `SystemExit` and passes
    straight through.
    """
    try:
        gate(args)
...
def state_is_a_guess(cfg):
    ...
    `home` out of a broken file would need the second parser this project
    deliberately deleted, so the gate says the directory it searched may be the
    wrong one instead of implying it looked where the claims are. HQ_HOME
    settles it, because the environment is read whatever the file does.
    """
...
def gate(args):
    ...
    print(
        ...
        f"    Coordinate first:  hq msg {owner} \"...\"   "
        f"(or have the owner run: hq release {args.branch})",
        file=sys.stderr,
    )
    sys.exit(1)
